...
Error descriptions that appear in normal code output have four parts:
...
- LogEntry - The text of the original error message from the log (or equivalent text).

Depending on the situation, all four parts might not be shown. Generally, the number is shown.
...
### Errors Specific to a Service

Some services might contain additional error output, as needed. Please see docs for the service.
...
-1      : Not an error but is information you probably want to know.
...
n01-n29 : This can vary by service, but this range is generally for errors generated by the service itself.
          That is, these errors might originate in the GDD service or in child processes and applications, 
          but they are detected, identified and reported by the GDD service.
...
256     : The script or program could not execute properly. This is likely to be accompanied by an exit
          code of '1'. 

299     : Multiple errors were detected.  They are usually in the file `error_logs/<project_id>_errors.txt` 
